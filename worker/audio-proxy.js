...
export default {
  async fetch(request, env, ctx) {
    ...
    // feed.xml : la route Cloudflare `www.prestopodcast.online/feed.xml` est
    // TOUJOURS attachée à ce worker (bd0859e a retiré le code, mais son
    // déploiement avait échoué, donc la route n'a jamais été supprimée).
    // Tant que cette route existe, Pages ne voit jamais la requête : sans ce
    // handler, le feed répond 404 et tous les abonnements podcast cassent.
    // Ne retirer ce bloc qu'APRÈS avoir supprimé la route côté Cloudflare.
...
// Utilisé uniquement par /feed.xml : un flux RSS doit être servi tel quel
// (les apps podcast ne suivent pas toujours une redirection), contrairement
// aux MP3 qui sont redirigés vers les releases GitHub pour économiser le quota.
